...
impl AlpacaApiBroker {
    ...
    pub async fn fetch_account(&self) -> Result<Account, BrokerError> {
        ...
        let api_account = resp.json::<AlpacaApiAccount>().await
    ...
    pub async fn fetch_positions(&self) -> Result<Vec<Position>, BrokerError> {
        ...
        let api_positions = resp.json::<Vec<AlpacaApiPosition>>().await
            .map_err(|e| BrokerError::Fail(e.to_string()))?;

        Ok(api_positions.into_iter().map(|p| p.into_position()).collect())
    }
    ...
    pub async fn submit_order(&self, order: &Order) -> Result<OrderId, BrokerError> {
        ...
        if !resp.status().is_success() {
            return Err(BrokerError::OrderRejected(resp.text().await.unwrap_or_default()));
        }

        let order_resp = resp.json::<AlpacaApiOrderResponse>().await
            .map_err(|e| BrokerError::Fail(e.to_string()))?;
