...
async fn test_position_crud() {
    ...
    // Create position
    storage.upsert_position("AAPL", 100.0, 150.0, 155.0).await.unwrap();
    ...
    // Update position
    storage.upsert_position("AAPL", 150.0, 152.0, 160.0).await.unwrap();
    let pos = storage.get_position("AAPL").await.unwrap().unwrap();
...
async fn test_trade_insert_and_query() {
    ...
    // Insert trades
    storage.insert_trade("AAPL", "buy", 100.0, 150.0, Some("order-1"), Some("momentum")).await.unwrap();
    storage.insert_trade("MSFT", "buy", 50.0, 300.0, Some("order-2"), Some("momentum")).await.unwrap();
    storage.insert_trade("AAPL", "sell", 100.0, 160.0, Some("order-3"), Some("momentum")).await.unwrap();
...
async fn test_signal_insert_and_query() {
    ...
    storage.insert_signal("AAPL", "buy", 0.8, "momentum").await.unwrap();
    storage.insert_signal("MSFT", "sell", 0.6, "mean_reversion").await.unwrap();
...
async fn test_account_snapshot() {
    ...
    storage.insert_account_snapshot(100_000.0, 80_000.0, 80_000.0).await.unwrap();
    storage.insert_account_snapshot(101_000.0, 81_000.0, 81_000.0).await.unwrap();
